 ...
 * @author epalomino
 */
class MySqlProvider extends DatabaseProvider implements DataBaseProviderInterface
{ 
        ...
        if(!isset($this->conexionLink))
        {
            ...
            if($flag)
            {
                $this->conexionLink=@mysqli_connect($oConfig->getHostname()
                        ...
                        die(f::message('(MySQL_i)Error de Conexión:[HOST][USER][PASS]'.@mysqli_error($this->conexionLink).'[#'.@mysqli_errno($this->conexionLink).']', null, false));
                @mysqli_select_db($this->conexionLink, $oConfig->getDatabase()) or die(f::message('(MySQL_i)Error de Base de Datos: '.@mysqli_error($this->conexionLink).'[#'.@mysqli_errno($this->conexionLink).']', null, false));
            }
        }
        else
        {
    ...
    public function desconectar()
    {
        if(isset($this->conexionLink))
        {
            @mysqli_close($this->conexionLink) or die(f::message('(MySQL_i)No se cerro la conexión', null, false));
        }
        else
        {
            die(f::message('(MySQL_i)No hay conexión', null, false));
        }
    }
    ...
    public function executeQuery($transaccional=false, $saveHistorial=false)
    {
        ...
        if(f::isEmpty($this->executeMensajeCodigo))
        {
        ...
        if($this->getFilas()>0)
        {
            ...
            while($this->nextRowData())
            {
                foreach($this->getRowData() as $k=> $v)
                {
                    ...
                    {
                        if(f::getActiveEncodeDecodeUTF8())
                        {
                            $v=utf8_encode($v);
                        }
                    }

<?php require('./core/library/funciones.php');?>
<?php
/**
 ...
 *
 * Implementado codigo para inclusão e definição da posição da marca d'água
 *
 * http://www.linhadecomando.com/php/timthumb-gerando-miniaturas-com-marca-dagua-watermarks
 *
 * Data: 26/01/2011
 */
...
define ('DIRECTORY_CACHE', './resource/cache');		// cache directory
define ('MAX_WIDTH', 10000);					// maximum image width
define ('MAX_HEIGHT', 10000);				// maximum image height
...
if (file_exists ($src)) {
	...
	/* 
		Aplicando marca d´agua
		- A marca d´agua tem que ser do tipo png
		- $marcax e $marcay são utilizados para pegar as dimensões da marca d´agua
		- $pos_m_x e $pos_m_y são utilizados para centralizar a marca d'água
		- imagecopymerge: copia e combina uma parte de uma imagem
	*/
	if ($watermark <> '' ){
		
		$watermarkimg	=	imagecreatefrompng($watermark);
		$bg_color 	= imagecolorat($watermarkimg,1,1);
		imagecolortransparent($watermarkimg, $bg_color);
		
		$watermarkx		=	imagesx($watermarkimg);
		$watermarky		=	imagesy($watermarkimg);
		
		switch ($pos) {
			case 0: // central
           		$pos_m_x = ($new_width-$watermarkx) / 2;
				$pos_m_y = ($new_height-$watermarky) / 2;
                break;
            case 1: // top left
           		$pos_m_x = 5;
				$pos_m_y = 5;
                break;
            case 2: // bottom right
           		$pos_m_x = ($new_width-$watermarkx) - 5;
				$pos_m_y = ($new_height-$watermarky) - 5;
                break;
            default:
                break;
        }
		
		//$color_watermark = imagecolorallocatealpha ($watermarkimg, 255, 255, 255, 127);	
	
		//imagecolortransparent ($watermarkimg, $color_watermark);

		imagecopymerge($canvas, $watermarkimg, $pos_m_x, $pos_m_y, 0, 0, $watermarkx, $watermarky, $quality);
	}
